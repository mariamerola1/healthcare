...
// * Sept 08, 2020  - for Healthcare demo  (HCAZ)
// Global variables
// This is modified to use GitHub and healthcare repo
...
def gitOrg = 'rbarosa'
def gitHost = '169.60.70.162'
// def gitHost = 'github.ibm.com'
def srcGitBranch = 'sandbox'
def srcGitRepo =     'git@'+gitHost+':'+gitOrg+'/healthcare.git'
//def srcGitRepo =   'git@github.ibm.com:rbarosa/healthcare.git'
// Example def srcGitRepo =   'git@'+gitHost+':'+gitOrg+'/nazare-demo-genapp.git'
//def adminGitRepo =   'git@'+gitHost+':'+gitOrg+'/healthcare.git'
...
// -------------------------------------------------------------------------
      stages { stage('Init') {steps { script {env.DBB_HOME = '/var/dbb/v.1.0.6'
        echo "Repository is at : /github.ibm.com/rbarosa/healthcare "
...
// -------------------------------------------------------------------------
 ...
 // added by Regi ---------- delete directory healthcare
 //  since the 3rd build does not take the change.. BUG?
                   sh(script: 'rm -rf ${WORKSPACE}/healthcare', returnStdout: true)
                   sh(script: 'rm -rf ${WORKSPACE}/healthcare@tmp', returnStdout: true)
 // ------------------------------------
                script {
                    dir('healthcare') {
...
// -------------------------------------------------------------------------
  ...
  sh "$DBB_HOME/bin/groovyz ${WORKSPACE}/healthcare/zAppBuild/build.groovy --logEncoding UTF-8 -w ${WORKSPACE} --application healthcare --sourceDir ${WORKSPACE}  --workDir ${WORKSPACE}/BUILD-${BUILD_NUMBER}  --hlq ${dbbHlq}.HEALTH --url $dbbUrl -pw ADMIN $buildType  $buildVerbose $buildExtraParams "
...
// -------------------------------------------------------------------------
		stage('Unit Tests') {
           ...
           echo "Perform ZUnit to check if all is working as it was"
            ...
            // ------ z/OS agent  ----------------------------------
					    node( agentLabel ) {	
		sh "$DBB_HOME/bin/groovyz  ${WORKSPACE}/healthcare/zAppBuild/zunit/ZUnitExecute.groovy -w ${WORKSPACE} --testConf ${WORKSPACE}/healthcare/application-conf/ --cc --outDir ${WORKSPACE}/BUILD-${BUILD_NUMBER} --hlq ${dbbHlq}.HEALTH  ${WORKSPACE}/BUILD-${BUILD_NUMBER}/buildList-1047.txt" 											
 					                       }
					              }
				      }
			      }
...
// -------------------------------------------------------------------------
   ...
   sh "$DBB_HOME/bin/groovyz ${WORKSPACE}/healthcare/zAppBuild/utilities/deploy.groovy --buztool  /apps/ucd/v7/bin/buztool.sh  --workDir ${WORKSPACE}/BUILD-${BUILD_NUMBER}  --component HC_CICS  "
                                           }
                                      }
                     }
                  }
